...
func TestMeteredUsesOneBoundedReadahead(t *testing.T) {
	p := New()
	...
	if ra.SeqThreshold != 6 || ra.Blocks != 2 || ra.Workers != 1 {
		t.Fatalf("metered link must use the guarded 2-block/1-worker policy, got %+v", ra)
	}
}
...
func TestForceClass(t *testing.T) {
	...
	if got := p.Readahead(); !got.Enabled || got.Blocks != 2 || got.Workers != 1 {
